...
struct CDijkstraTransportationPlanner::SImplementation {
    ...
    SImplementation(std::shared_ptr<SConfiguration> config) {
        ...
        for (std::size_t i = 0; i < DStreetMap->WayCount(); ++i) {
            ...
            for (std::size_t j = 1; j < way->NodeCount(); ++j) {
                // Assuming there's a way to get nodes from a way, fix according to the interface
                // Use NodeByIndex instead of GetNodeByIndex
                auto node1ID = way->NodeID(j-1);
                auto node2ID = way->NodeID(j);
    ...
    double CalculateDistance(std::shared_ptr<CStreetMap::SNode> node1, 
        ...
        // Use GetLatitude and GetLongitude instead of Latitude and Longitude
        double lat1 = node1->GetLatitude() * DegreesToRadians;
        double lon1 = node1->GetLongitude() * DegreesToRadians;
        double lat2 = node2->GetLatitude() * DegreesToRadians;
        double lon2 = node2->GetLongitude() * DegreesToRadians;
