...
#include "KMLWriter.h"
#include "DataSink.h" // Ensure this is included both here and in the header
#include <limits>
#include <algorithm>
...
struct CDijkstraTransportationPlanner::SImplementation {
    ...
    SImplementation(std::shared_ptr<SConfiguration> config) {
        ...
        for (std::size_t i = 0; i < DStreetMap->WayCount(); ++i) {
            ...
            for (std::size_t j = 1; j < way->NodeCount(); ++j) {
                ...
                // Use GeographicUtils for distance calculation
    ...
    bool CreateKMLFile(std::shared_ptr<CDataSink> sink, const std::vector<TNodeID> &path) {
        ...
        // Create KML writer
        std::string title = "Transportation Plan Path";
        std::string description = "Path generated by DijkstraTransportationPlanner";
        CKMLWriter kmlWriter(sink, title, description);

        // Create styles
        kmlWriter.CreatePointStyle("startPoint", 0xFF00FF00); // Green point
        kmlWriter.CreatePointStyle("endPoint", 0xFF0000FF);   // Red point
        kmlWriter.CreateLineStyle("pathLine", 0xFF0000FF, 4); // Red line, 4 pixels wide

        // Add start point
        TNodeID startNodeID = path.front();
        auto startNodeIter = DNodeIDToIndex.find(startNodeID);
        if (startNodeIter != DNodeIDToIndex.end()) {
            auto startNode = DNodes[startNodeIter->second];
            std::string pointName = "Start";
            std::string pointDesc = "Starting location";
            if (startNode->HasAttribute("name")) {
                pointName += " - " + startNode->GetAttribute("name");
            }
            kmlWriter.CreatePoint(pointName, pointDesc, "startPoint", startNode->Location());
        }

        // Add end point
        TNodeID endNodeID = path.back();
        auto endNodeIter = DNodeIDToIndex.find(endNodeID);
        if (endNodeIter != DNodeIDToIndex.end()) {
            auto endNode = DNodes[endNodeIter->second];
            std::string pointName = "End";
            std::string pointDesc = "Destination location";
            if (endNode->HasAttribute("name")) {
                pointName += " - " + endNode->GetAttribute("name");
            }
            kmlWriter.CreatePoint(pointName, pointDesc, "endPoint", endNode->Location());
        }

        // Create the path line
        std::vector<CStreetMap::TLocation> pathLocations;
        for (const auto &nodeID : path) {
            auto nodeIter = DNodeIDToIndex.find(nodeID);
            if (nodeIter != DNodeIDToIndex.end()) {
                pathLocations.push_back(DNodes[nodeIter->second]->Location());
            }
        }

        if (!pathLocations.empty()) {
            kmlWriter.CreatePath("Planned Route", "pathLine", pathLocations);
        }
...
bool CDijkstraTransportationPlanner::CreateKMLFile(std::shared_ptr<CDataSink> sink, const std::vector<TNodeID> &path) {
    return DImplementation->CreateKMLFile(sink, path);
}