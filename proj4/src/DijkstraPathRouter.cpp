...
struct CDijkstraTransportationPlanner::SImplementation {
    ...
    void BuildShortestPathRouter() {
        ...
        for(size_t i = 0; i < StreetMap->WayCount(); ++i) {
            ...
            for(size_t j = 0; j < Way->AttributeCount(); ++j) {
                auto Attribute = Way->GetAttribute(j);
                if(Attribute.first == "oneway" && Attribute.second == "yes") {
                    IsOneWay = true;
                    break;
                }
            }
            ...
            for(size_t j = 1; j < Way->NodeCount(); ++j) {
                ...
                auto PrevLoc = std::make_pair(PrevNode->Latitude(), PrevNode->Longitude());
                auto CurrLoc = std::make_pair(CurrNode->Latitude(), CurrNode->Longitude());
    ...
    void BuildFastestPathRouter() {
        ...
        for(size_t i = 0; i < StreetMap->WayCount(); ++i) {
            ...
            for(size_t j = 0; j < Way->AttributeCount(); ++j) {
                ...
                else if(Attribute.first == "maxspeed" && Attribute.second.find("mph") != std::string::npos) {
                    try {
                        SpeedLimit = std::stod(Attribute.second.substr(0, Attribute.second.find("mph")));
                    }
                    catch(...) {
    ...
    bool GetPathDescription(const std::vector<TTripStep> &path, std::vector<std::string> &desc) const {
        ...
        auto StartLocation = std::make_pair(FirstNode->Latitude(), FirstNode->Longitude());
        ...
        for(size_t i = 0; i < path.size() - 1; ++i) {
            ...
            auto CurrentLocation = std::make_pair(CurrentNode->Latitude(), CurrentNode->Longitude());
            auto NextLocation = std::make_pair(NextNode->Latitude(), NextNode->Longitude());
        ...
        auto EndLocation = std::make_pair(LastNode->Latitude(), LastNode->Longitude());
